...
void sendToWeb(int pulse, int spo2) {
    Serial.print("PULSE:");
    Serial.print(pulse);
    Serial.print(":SPO2:");
    Serial.println(spo2);
...
void loop()  {
    ...
    if (irValue<5000) {
        ...
        if (draw_Red ? beatRed : beatIR){
            ...
            if (btpm > 50 && btpm < 90) beatAvg = bpm.filter((int16_t)btpm);
            lastBeat = now; 
            ...
            if ((RX100>=0) && (RX100<184)) {
                ...
                // Validate SPO2 is in reasonable range
                if (SPO2 < 70 || SPO2 > 100) {
                    // Use formula-based value if table lookup is out of range
                    SPO2 = SPO2f;
                }
            ...
            // Only send with heartbeat if both values are valid
            if (beatAvg > 0 && SPO2 > 0) {
                sendToWeb(beatAvg, SPO2);
            }
        }
        ...
        if (now-displaytime>50) {
            ...
            // Send current values to web regardless of heartbeat detection
            if (beatAvg > 0 && SPO2 > 0) {
                sendToWeb(beatAvg, SPO2);
            }
        }
